...
A photograph of a large, modern, light-colored building with many windows, likely a university or research facility, surrounded by trees and a clear blue sky.
...
<!-- Tipo: generico -->
> **[Descrição de imagem]** A photograph of a large, modern, light-colored building with many windows, likely a university or research facility, surrounded by trees and a clear blue sky.
<!-- /IMAGE_DESCRIPTION -->
### Sinopse
...
The figure shows a scatter plot with two clusters of data points. The left cluster consists of four blue points and a red centroid. The right cluster consists of eight blue points and a pink centroid. Dashed lines connect each data point to its respective centroid, illustrating the SSE (Sum of Squared Errors) for each cluster. The axes are labeled from 1 to 10.

| Cluster | Centroid (X, Y) | Data Points (X, Y)                                                     |
|---------|-----------------|------------------------------------------------------------------------|
| Left    | (2, 4)          | (1, 3), (1, 4), (2, 5), (3, 4)                                         |
| Right   | (8.5, 7)        | (7.5, 6), (7.5, 7), (7.5, 8), (8, 9), (9, 9), (9, 8), (9, 6), (9.5, 5) |

A scatter plot illustrating two clusters of data points. The left cluster has a red centroid and four blue points. The right cluster has a pink centroid and eight blue points. Dashed lines connect each point to its centroid, representing the SSE (Sum of Squared Errors).
...
- **Atividade 1:** Calcule os índices TP, TN, FP e FN para as classes Iris-Virginica e Iris-Setosa
...
The left diagram shows a Venn diagram with two overlapping circles. The left circle is yellow and the right circle is blue with diagonal hatching. The intersection is labeled "Relevant items - retrieved". The part of the yellow circle not in the intersection is labeled "Relevant items - not retrieved". The part of the blue circle not in the intersection is labeled "Irrelevant items - retrieved".

The right diagram shows a Venn diagram with two overlapping circles. The left circle is yellow and labeled "A". The right circle is blue with diagonal hatching and labeled "C".
...
Diagram illustrating Precision with two Venn diagrams. The left diagram shows a yellow circle (relevant items) and a blue circle (retrieved items). The intersection is labeled 'Relevant items - retrieved'. The part of the yellow circle not in the intersection is labeled 'Relevant items - not retrieved'. The part of the blue circle not in the intersection is labeled 'Irrelevant items - retrieved'. The right diagram shows a yellow circle labeled 'A' (relevant items retrieved) and a blue circle labeled 'C' (irrelevant items retrieved). The formula for Precision is given as \frac{A}{A+C} \times 100%.
...
Venn diagram illustrating Precision. A yellow circle labeled 'A' represents relevant records retrieved. A blue circle labeled 'C' represents irrelevant records retrieved. The intersection of the two circles is shaded with diagonal lines. The formula for Precision is given as (A / (A+C)) \* 100%.
...
The left diagram shows a yellow circle (Relevant items) and a blue circle (Retrieved items). The intersection is labeled "Relevant items - retrieved". The part of the yellow circle not in the intersection is labeled "Relevant items - not retrieved". The part of the blue circle not in the intersection is labeled "Irrelevant items - retrieved".

The right diagram shows a yellow circle divided into two parts: B (Number of relevant records not retrieved) and A (Number of relevant records retrieved).
...
Venn diagram illustrating Recall. A yellow circle is divided into two parts: B (unshaded) and A (hatched). B is labeled 'Number of relevant records not retrieved.' A is labeled 'Number of relevant records retrieved.' The formula for Recall is given as RECALL: \frac{A}{A+B} \times 100%.
...
<!-- Tipo: generico -->
> **[Descrição de imagem]** Venn diagram illustrating Recall. A yellow circle is divided into two parts: B (unshaded) and A (hatched). B is labeled 'Number of relevant records not retrieved.' A is labeled 'Number of relevant records retrieved.'...
<!-- /IMAGE_DESCRIPTION -->
##### Classificação: Recall
